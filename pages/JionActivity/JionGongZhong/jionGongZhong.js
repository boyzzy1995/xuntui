...
Page({
  ...
  onLoad: function(options) {
    ...
    wx.downloadFile({
      ...
      success: function(resp) {
        wx.hideLoading()
        me.createNewImg(resp.tempFilePath, options.title, options.endTime, options.ewm);
      }
    })
  },
  ...
  setNameText: function(context, title) {
    ...
    var textFir = "砍价:" + title;
    context.setFontSize(22);
    context.setTextAlign("left");
    context.setFillStyle("#000000");
    context.fillText(textFir, size.w * 0.05, size.h * 0.84);
    context.stroke();
  },
  //将1绘制到canvas的固定
  setTimeText: function(context, endtime) {
    ...
    context.fillText(textFir, size.w * 0.05, size.h * 0.94);
    context.stroke();
  },

  //将canvas转换为图片保存到本地，然后将图片路径传给image图片的src
  createNewImg: function(url, title, endtime , ewm) {
    var that = this;
    var size = that.setCanvasSize();
    ...
    context.drawImage(imageProduct, size.w * 0.03, size.h * 0.038, size.w * 0.94, size.h * 0.7);
    context.drawImage(imageEwm, size.w * 0.79, size.h * 0.75, size.w * 0.17, size.w * 0.17);
